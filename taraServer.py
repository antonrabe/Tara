...
def get_events():
    ...
    for i in range(0, evt_list_len):
        event_str += events[i]['index'] + "|" + events[i]['venue'] + "|" + events[i]['time'] + "|" + events[i]['category'] + "|" + events[i]['numPeople'] + ";"
...
def upload_events():
    ...
    if (request.method == 'POST'):
        ...
        if numPeople == "":
            return "Failed!"
        events.append({ 'index':index, 'venue':venue, 'time':time, 'category':category, 'numPeople':numPeople })
    return "Failed!"
